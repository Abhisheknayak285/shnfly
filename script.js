// --- script.js (Includes Auth, Game Logic, Dynamic Rocket, Potential Win, Backend Balance Sync, Continuous Loop Simulation, Wallet Restriction, Dynamic Player Count) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const BETTING_DURATION = 4000; // 7 seconds
    ...
    const playerCountDisplay = document.getElementById('playerCount'); // Player count element
    ...
    let playerCountIntervalId = null; // To store the interval ID
    let currentPlayerCount = 33; // Initial count (can be random too)
    const MIN_PLAYERS = 15; // Minimum players shown
    const MAX_PLAYERS = 180; // Maximum players shown
    const PLAYER_COUNT_UPDATE_INTERVAL = 5000; // Update every 5 seconds (5000ms)
    const MAX_PLAYER_CHANGE = 7; // Max change (+/-) per update
    ...
    function showAuth() {
        gameState = 'IDLE';
        stopGame(); // Make sure intervals (including player count) are stopped
    ...
    function showGame() {
        ...
        initializeGameUI(); // This now also starts the player count interval
        loadSoundPreference();
    ...
    async function fetchUserBalance(username) {
        if (!username || GAS_WEB_APP_URL === 'YOUR_DEPLOYED_WEB_APP_URL') {
            console.error("Cannot fetch balance: Username missing or GAS URL not set.");
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`HTTP error fetching balance! status: ${response.status}`); return DEFAULT_BALANCE;
            }
            const result = await response.json();
            if (result.status === 'success' && result.balance !== undefined && !isNaN(parseFloat(result.balance))) {
                console.log("Balance successfully fetched:", result.balance);
                return parseFloat(result.balance);
            } else {
                console.error("Failed to fetch balance from backend:", result.message || "User not found or invalid balance.");
                return DEFAULT_BALANCE;
            }
        } catch (error) {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(GAS_WEB_APP_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ action: 'login', username, password }) });
            if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
            const result = await response.json();
            if (result.status === 'success') {
                sessionStorage.setItem('crashGameUser', result.user.username);
                if (result.user && result.user.balance !== undefined && !isNaN(parseFloat(result.user.balance))) {
                    balance = parseFloat(result.user.balance); console.log(`Balance loaded from backend: ${balance}`);
                } else {
                    balance = DEFAULT_BALANCE; console.warn("Balance not found or invalid in login response, using default.");
                }
                showGame();
            } else { displayAuthError('login', result.message || 'Login failed.'); }
        } catch (error) { console.error('Login Error:', error); displayAuthError('login', 'Login failed. Check connection or try again.'); }
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const result = await response.json();
            if (result.status === 'success') {
                alert('Registration successful! Please log in.'); showLoginForm();
    ...
    logoutButton.addEventListener('click', () => {
        sessionStorage.removeItem('crashGameUser');
        showAuth(); console.log('User logged out.');
    });

    function showLoginForm() {
        loginForm.classList.remove('hidden'); registerForm.classList.add('hidden');
        clearAuthErrors(); loginForm.reset();
    }
    function showRegisterForm() {
        loginForm.classList.add('hidden'); registerForm.classList.remove('hidden');
        clearAuthErrors(); registerForm.reset();
    }
    ...
    async function saveBalanceToBackend(username, balanceToSave) {
        if (!username || GAS_WEB_APP_URL === 'YOUR_DEPLOYED_WEB_APP_URL') { console.error("Cannot save balance: Username missing or GAS URL not set."); return; }
        const numericBalance = parseFloat(balanceToSave);
        if (isNaN(numericBalance)) { console.error("Cannot save balance: Invalid balance value.", balanceToSave); return; }
        console.log(`Attempting to save balance: ${numericBalance} for user: ${username}`);
        try {
            fetch(GAS_WEB_APP_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ action: 'updateBalance', username: username, newBalance: numericBalance }) })
                .then(response => response.json())
                .then(result => {
                    if (result.status === 'success') { console.log("Balance saved successfully on backend."); }
                    else { console.error("Failed to save balance on backend:", result.message); }
                })
                .catch(error => { console.error('Error saving balance:', error); });
        } catch (error) { console.error('Error initiating balance save:', error); }
    }
    ...
    function setBetControlsDisabled(disabled) {
        ...
        quickBetBtns.forEach(btn => btn.disabled = disabled);
        if (autoBetAmountInput) autoBetAmountInput.disabled = disabled;
        if (autoCashoutMultiplierInput) autoCashoutMultiplierInput.disabled = disabled;
        if (autoBetEnabledCheckbox) autoBetEnabledCheckbox.disabled = disabled;
        if (autoCashoutEnabledCheckbox) autoCashoutEnabledCheckbox.disabled = disabled;
    }

    function updateMainActionButton(state, text = '', value = null) {
        mainActionButton.disabled = false;
        mainActionButton.className = '';
        switch (state) {
            case 'idle': mainActionButton.classList.add('state-idle'); mainActionButton.textContent = text || 'Place Bet'; break;
            case 'waiting_start': mainActionButton.classList.add('state-waiting_start'); mainActionButton.textContent = text || 'Waiting...'; mainActionButton.disabled = true; break;
            case 'running': mainActionButton.classList.add('state-running'); mainActionButton.textContent = text || `Cash Out @ ${value || currentMultiplier}x`; break;
            case 'cashed_out': mainActionButton.classList.add('state-cashed_out'); mainActionButton.textContent = text || `Cashed Out @ ${value}x`; mainActionButton.disabled = true; break;
            case 'crashed': mainActionButton.classList.add('state-crashed'); mainActionButton.textContent = text || 'Crashed'; mainActionButton.disabled = true; break;
            default: mainActionButton.classList.add('state-idle'); mainActionButton.textContent = 'Place Bet';
        }
    }

    // Function to update player count dynamically
    function updateDynamicPlayerCount() {
        if (!playerCountDisplay) return; // Make sure the element exists

        // Calculate random change (+/- MAX_PLAYER_CHANGE)
        const change = Math.floor(Math.random() * (MAX_PLAYER_CHANGE * 2 + 1)) - MAX_PLAYER_CHANGE;

        let newCount = currentPlayerCount + change;

        // Clamp the count between MIN and MAX players
        newCount = Math.max(MIN_PLAYERS, newCount);
        newCount = Math.min(MAX_PLAYERS, newCount);

        currentPlayerCount = newCount; // Update the current count state
        playerCountDisplay.textContent = currentPlayerCount; // Update the display
    }
    ...
    function closeAllPopups() {
        ...
        closePopup(autoPopup);
        closePopup(walletPopup);
    }

    // --- Wallet UI Logic (Placeholders) ---
    function setupWalletUI() {
        showAddMoneyBtn.addEventListener('click', () => { addMoneySection.classList.add('active'); withdrawMoneySection.classList.remove('active'); showAddMoneyBtn.classList.add('active'); showWithdrawMoneyBtn.classList.remove('active'); resetWalletForms(); });
        showWithdrawMoneyBtn.addEventListener('click', () => { addMoneySection.classList.remove('active'); withdrawMoneySection.classList.add('active'); showAddMoneyBtn.classList.remove('active'); showWithdrawMoneyBtn.classList.add('active'); resetWalletForms(); });
        paymentMethodSelect.addEventListener('change', () => { const method = paymentMethodSelect.value; upiDetailsDiv.classList.add('hidden'); qrCodeDetailsDiv.classList.add('hidden'); transactionIdSection.classList.add('hidden'); submitDepositButton.disabled = true; depositStatus.textContent = ''; if (method === 'upi') { upiDetailsDiv.classList.remove('hidden'); transactionIdSection.classList.remove('hidden'); } else if (method === 'qr') { qrCodeDetailsDiv.classList.remove('hidden'); transactionIdSection.classList.remove('hidden'); } });
        transactionIdInput.addEventListener('input', () => { submitDepositButton.disabled = transactionIdInput.value.trim().length < 10; });
        submitDepositButton.addEventListener('click', () => { const txnId = transactionIdInput.value.trim(); depositStatus.textContent = 'Processing deposit...'; depositStatus.className = 'wallet-status pending'; submitDepositButton.disabled = true; setTimeout(() => { const success = Math.random() > 0.2; if (success) { depositStatus.textContent = 'Deposit successful! (Demo)'; depositStatus.className = 'wallet-status success'; const demoAmount = Math.floor(Math.random() * 500) + 100; balance += demoAmount; updateBalanceDisplay(); const currentUser = sessionStorage.getItem('crashGameUser'); if (currentUser) { saveBalanceToBackend(currentUser, balance); } } else { depositStatus.textContent = 'Deposit verification failed. (Demo)'; depositStatus.className = 'wallet-status error'; submitDepositButton.disabled = false; } transactionIdInput.value = ''; paymentMethodSelect.value = ''; upiDetailsDiv.classList.add('hidden'); qrCodeDetailsDiv.classList.add('hidden'); transactionIdSection.classList.add('hidden'); submitDepositButton.disabled = true; }, 2500); });
        submitWithdrawalButton.addEventListener('click', () => { const amount = parseFloat(withdrawAmountInput.value); const upiId = userUpiIdInput.value.trim(); withdrawalStatus.textContent = ''; withdrawalStatus.className = 'wallet-status'; if (isNaN(amount) || amount < 100) { withdrawalStatus.textContent = 'Minimum withdrawal is ₹100.'; withdrawalStatus.className = 'wallet-status error'; return; } if (amount > balance) { withdrawalStatus.textContent = 'Insufficient balance.'; withdrawalStatus.className = 'wallet-status error'; return; } if (!upiId || !upiId.includes('@')) { withdrawalStatus.textContent = 'Please enter a valid UPI ID.'; withdrawalStatus.className = 'wallet-status error'; return; } withdrawalStatus.textContent = 'Processing withdrawal request...'; withdrawalStatus.className = 'wallet-status pending'; submitWithdrawalButton.disabled = true; setTimeout(() => { balance -= amount; updateBalanceDisplay(); const currentUser = sessionStorage.getItem('crashGameUser'); if (currentUser) { saveBalanceToBackend(currentUser, balance); } withdrawalStatus.textContent = 'Withdrawal request submitted. (Demo)'; withdrawalStatus.className = 'wallet-status success'; withdrawAmountInput.value = ''; userUpiIdInput.value = ''; submitWithdrawalButton.disabled = false; }, 2000); });
        closeWalletButton.addEventListener('click', () => closePopup(walletPopup));
    }
    function resetWalletForms() {
        paymentMethodSelect.value = ''; upiDetailsDiv.classList.add('hidden'); qrCodeDetailsDiv.classList.add('hidden'); transactionIdSection.classList.add('hidden'); transactionIdInput.value = ''; submitDepositButton.disabled = true; depositStatus.textContent = ''; depositStatus.className = 'wallet-status'; withdrawAmountInput.value = ''; userUpiIdInput.value = ''; withdrawalStatus.textContent = ''; withdrawalStatus.className = 'wallet-status'; submitWithdrawalButton.disabled = false;
    }

    // --- Auto Play Logic ---
    function setupAutoPlay() {
        autoBetEnabledCheckbox.addEventListener('change', (e) => { autoBetActive = e.target.checked; console.log('Auto Bet Active:', autoBetActive); if (autoBetActive && gameState === 'BETTING') { handleAutoPlayNextRound(); } });
        autoCashoutEnabledCheckbox.addEventListener('change', (e) => { autoCashoutActive = e.target.checked; console.log('Auto Cashout Active:', autoCashoutActive); });
        closeAutoButton.addEventListener('click', () => closePopup(autoPopup));
    }
    function handleAutoPlayNextRound() {
        if (autoBetActive && gameState === 'BETTING' && !autoNextBetPlaced) {
            const autoBetAmount = parseInt(autoBetAmountInput.value);
            if (isNaN(autoBetAmount) || autoBetAmount <= 0) { console.warn("AutoPlay: Invalid bet amount set."); autoBetEnabledCheckbox.checked = false; autoBetActive = false; return; }
            if (autoBetAmount > balance) { console.warn("AutoPlay: Insufficient balance for auto bet."); return; }
            betAmountInput.value = autoBetAmount; console.log(`AutoPlay: Placing bet of ${autoBetAmount}`); placeBet(); autoNextBetPlaced = true;
        }
    }
    ...
    function toggleSound() {
        ...
        localStorage.setItem('crashGameSound', soundEnabled);
        updateSoundIcon(); console.log('Sound Enabled:', soundEnabled);
    }
    function updateSoundIcon() {
        if (soundEnabled) { menuSoundIcon.textContent = '🔊'; menuSoundIcon.classList.remove('muted'); }
        else { menuSoundIcon.textContent = '🔇'; menuSoundIcon.classList.add('muted'); }
    }
    function loadSoundPreference() {
        const savedSoundPref = localStorage.getItem('crashGameSound');
        if (savedSoundPref !== null) { soundEnabled = savedSoundPref === 'true'; }
        updateSoundIcon();
    }

    // --- History Management ---
    function addHistory(multiplierValue) {
        const newRecord = { multiplier: parseFloat(multiplierValue), timestamp: Date.now() };
        gameHistory.unshift(newRecord); updateHistoryDisplay();
    }
    function updateHistoryDisplay() {
        mainHistoryDisplay.innerHTML = '';
        if (gameHistory.length === 0) { mainHistoryDisplay.innerHTML = '<span class="history-empty-message">No rounds played yet.</span>'; return; }
        const itemsToDisplay = gameHistory.slice(0, MAX_HISTORY_ITEMS_MAIN);
        itemsToDisplay.forEach(record => {
            ...
            item.innerHTML = `<span class="history-item-multiplier">${multiplier.toFixed(2)}x</span>`;
            mainHistoryDisplay.appendChild(item);
        });
    }
    ...
    function resetRound() {
        ...
        initializeGameUI(); // Re-initialize UI, including player count update start
    }
    ...
    function mainGameCycle() {
        ...
        switch (gameState) {
            ...
            case 'RUNNING':
                ...
                // The gameTick interval handles updates in this state
                break;
    ...
    function initializeGameUI() {
        ...
        // Start/Reset Player Count Simulation
        clearInterval(playerCountIntervalId); // Clear any previous interval
        updateDynamicPlayerCount(); // Set initial count immediately
        playerCountIntervalId = setInterval(updateDynamicPlayerCount, PLAYER_COUNT_UPDATE_INTERVAL); // Start the interval
    }
    ...
    function generateCrashPoint() {
        ...
        if (r < 0.02) { crash = 1.00; }
        else if (r < 0.5) { crash = 1.01 + Math.random() * 0.98; }
        else if (r < 0.8) { crash = 2 + Math.random() * 3; }
        else if (r < 0.95) { crash = 5 + Math.random() * 5; }
        else { crash = 10 + Math.random() * 20; }
        return Math.max(1.00, crash);
    }

    function calculateMultiplier(timeMillis) {
        const base = 1.00; const growthRate = 0.08; const exponent = 1.3;
        const multiplier = base + growthRate * Math.pow(timeMillis / 1000, exponent);
    ...
    function gameTick() {
        ...
        if (Math.abs(newMultiplier - currentMultiplier) >= 0.01) {
            ...
            if (currentBet && !hasCashedOut) {
                updateMainActionButton('running', null, currentMultiplier.toFixed(2));
            }
        }
        // Rocket Position Update
        const initialPosPercent = 5; const speedFactor = 7; const accelerationFactor = 1.25; const maxPosPercent = 88;
        let positionPercentage = initialPosPercent + speedFactor * Math.pow(timeElapsed / 1000, accelerationFactor);
        positionPercentage = Math.min(maxPosPercent, positionPercentage);
        const initialLeftPercent = 10; const maxLeftPercent = 90; const diagonalFactor = 0.9;
        let leftPercentage = initialLeftPercent + (positionPercentage - initialPosPercent) * diagonalFactor;
        leftPercentage = Math.min(maxLeftPercent, leftPercentage);
        rocket.style.bottom = `${positionPercentage}%`;
        rocket.style.left = `${leftPercentage}%`;

        // Potential Win Update
        if (multiplierChanged && currentBet && !hasCashedOut) {
            const potentialWin = currentBet.amount * currentMultiplier;
            potentialWinDisplay.textContent = potentialWin.toFixed(2);
        }
        // Auto Cashout Check
        if (autoCashoutActive && currentBet && !hasCashedOut) {
            const autoTarget = parseFloat(autoCashoutMultiplierInput.value) || 2.00;
            if (currentMultiplier >= autoTarget) {
                cashOut();
            }
        }
    }
    ...
    function startGame() {
        ...
        console.log('Starting game RUNNING phase...');
        gameState = 'RUNNING'; gameStatusDisplay.textContent = '🚀 Rocket Launched!';
        crashPoint = generateCrashPoint(); gameStartTime = Date.now(); timeElapsed = 0; currentMultiplier = 1.00;
        ...
        if (currentBet) {
            ...
            potentialWinDisplay.textContent = (currentBet.amount * 1.00).toFixed(2);
            potentialWinDisplay.classList.remove('hidden');
        } else {
            updateMainActionButton('running', 'In Progress', '1.00');
        ...
        cloudsBackground.classList.add('clouds-active');
        setBetControlsDisabled(true);
    ...
    function stopGame() {
        ...
        clearTimeout(mainLoopTimeoutId); mainLoopTimeoutId = null;
        clearInterval(countdownIntervalId); countdownIntervalId = null;

        // Stop player count updates
        clearInterval(playerCountIntervalId);
        playerCountIntervalId = null; // Reset the ID

        console.log('Game loop and cycle stopped.');
    }

    function crashGame() {
        if (gameState === 'ENDED') return; // Prevent multiple crashes
        clearInterval(gameInterval); gameInterval = null;
        const finalMultiplier = Math.max(1.00, crashPoint); currentMultiplier = finalMultiplier; // Ensure multiplier shows crash point
        ...
        if (currentBet && !hasCashedOut) {
            ...
            if (currentUser) { saveBalanceToBackend(currentUser, balance); } // Save balance even on loss (no change)
        }
        betAmountInput.classList.remove('hidden'); potentialWinDisplay.classList.add('hidden'); potentialWinDisplay.textContent = '';
        currentBet = null; // Reset bet status
    ...
    function placeBet() {
        if (gameState !== 'BETTING') {
            gameStatusDisplay.textContent = "Betting closed. Wait for next round.";
            setTimeout(() => { if (gameState !== 'BETTING') gameStatusDisplay.textContent = "Wait for next round..."; }, 2000);
            return;
        }
        ...
        balance -= betValue;
        currentBet = { amount: betValue };
        hasCashedOut = false; // Reset cashout status for the new bet
        updateBalanceDisplay();
        gameStatusDisplay.textContent = `Bet Placed: ₹${betValue}. Waiting for launch...`;
        updateMainActionButton('waiting_start', 'Bet Placed');
        setBetControlsDisabled(true);
    ...
    function setupEventListeners() {
        ...
        mainActionButton.addEventListener('click', () => {
            if (gameState === 'BETTING') { placeBet(); }
            else if (gameState === 'RUNNING' && currentBet && !hasCashedOut) { cashOut(); }
        });
        ...
        menuWalletBtn.addEventListener('click', () => {
            if (gameState === 'RUNNING' || gameState === 'STARTING') { // Prevent opening during critical phases
                const originalStatus = gameStatusDisplay.textContent;
                gameStatusDisplay.textContent = "Please wait for the round to end.";
                setTimeout(() => { if (gameState === 'RUNNING' || gameState === 'STARTING') { /* Optionally revert status if still in same state */ } }, 2500);
